...
function classifyColumn(mysqlType: string): ColType {
  ...
  if (
    /^(int|bigint|smallint|tinyint|mediumint|float|double|decimal|numeric|real)/.test(
      t,
    )
  )
    return "numeric";
  if (/^(date|datetime|timestamp|time|year)/.test(t)) return "temporal";
  return "categorical";
}
...
${prompt}`;
  ...
  try {
    ...
    if (!response.ok)
      return "AI insight unavailable — service is not responding.";
...
function buildLineChart(
  ...
  const sorted = [...rows].sort((a, b) =>
    a[orderCol.name] < b[orderCol.name]
      ? -1
      : a[orderCol.name] > b[orderCol.name]
        ? 1
        : 0,
  ...
  const maxPts = 20;
  let sampled = sorted;
  if (sorted.length > maxPts) {
    const step = Math.floor(sorted.length / maxPts);
    sampled = sorted.filter((_, i) => i % step === 0).slice(0, maxPts);
  }
  return sampled.map((row) => ({
    name: String(row[orderCol.name] ?? ""),
...
function buildScatterChart(
  ...
  const sampled =
    rows.length > maxPts
      ? rows.filter((_, i) => i % Math.ceil(rows.length / maxPts) === 0)
      : rows;
  return sampled.map((row, i) => ({
    name: `Point ${i + 1}`,
    x: parseFloat(row[n1.name]) || 0,
    y: parseFloat(row[n2.name]) || 0,
    value: parseFloat(row[n2.name]) || 0,
  }));
}
...
function buildStackedBarChart(
  ...
  for (const row of rows) {
    const p = String(row[c1.name] ?? "Unknown"),
      s = String(row[c2.name] ?? "Unknown");
...
function buildRadarChart(
  ...
  for (const row of rows) {
    ...
    for (const nc of numCols)
      groups[key].sums[nc.name] =
        (groups[key].sums[nc.name] || 0) + (parseFloat(row[nc.name]) || 0);
  }
  const topCats = Object.entries(groups)
    .sort((a, b) => b[1].count - a[1].count)
    .slice(0, 5);
  return numCols.slice(0, 6).map((nc) => {
    const pt: ChartDataPoint = { name: nc.name, value: 0 };
    for (const [catName, group] of topCats) {
      const avg =
        Math.round(((group.sums[nc.name] || 0) / group.count) * 100) / 100;
      pt[catName] = avg;
      pt.value = avg;
    }
    return pt;
...
function pickBarTable(tables: TableProfile[]): TableProfile | null {
  ...
  return (
    tables.find(
      (t) => t.categoricalCols.length > 0 && t.numericCols.length > 0,
    ) ||
    tables[0] ||
    null
  );
...
function pickPieTable(
  ...
  return (
    tables.find(
      (t) => t.categoricalCols.length > 0 && !used.has(t.tableName),
    ) ||
    tables.find((t) => t.categoricalCols.length > 0) ||
    tables[0] ||
    null
  );
}

function pickLineTable(
  tables: TableProfile[],
  used: Set<string>,
): TableProfile | null {
  ...
  return (
    tables.find(
      (t) =>
        t.temporalCols.length > 0 &&
        t.numericCols.length > 0 &&
        !used.has(t.tableName),
    ) ||
    tables.find((t) => t.temporalCols.length > 0 && t.numericCols.length > 0) ||
    tables.find((t) => t.numericCols.length > 0 && !used.has(t.tableName)) ||
    tables.find((t) => t.numericCols.length > 0) ||
    tables[0] ||
    null
  );
}

function pickScatterTable(
  tables: TableProfile[],
  used: Set<string>,
): TableProfile | null {
  // Needs: 2+ numeric columns
  return (
    tables.find((t) => t.numericCols.length >= 2 && !used.has(t.tableName)) ||
    tables.find((t) => t.numericCols.length >= 2) ||
    tables[0] ||
    null
  );
}

function pickStackedTable(
  tables: TableProfile[],
  used: Set<string>,
): TableProfile | null {
  ...
  return (
    tables.find(
      (t) =>
        t.categoricalCols.length >= 2 &&
        t.numericCols.length > 0 &&
        !used.has(t.tableName),
    ) ||
    tables.find(
      (t) =>
        t.categoricalCols.length >= 1 &&
        t.numericCols.length >= 2 &&
        !used.has(t.tableName),
    ) ||
    tables.find(
      (t) => t.categoricalCols.length >= 2 && t.numericCols.length > 0,
    ) ||
    tables[0] ||
    null
  );
...
function pickRadarTable(
  ...
  return (
    tables.find(
      (t) =>
        t.categoricalCols.length > 0 &&
        t.numericCols.length >= 2 &&
        !used.has(t.tableName),
    ) ||
    tables.find((t) => t.numericCols.length >= 3 && !used.has(t.tableName)) ||
    tables.find(
      (t) => t.categoricalCols.length > 0 && t.numericCols.length >= 2,
    ) ||
    tables[0] ||
    null
  );
...
export async function generateReport(): Promise<ReportData> {
  ...
  try {
    ...
    for (const tr of tablesToProfile) {
      ...
      const [dataRows] = (await userPool.query(
        `SELECT * FROM \`${tName}\` LIMIT 500`,
      )) as [any[], any];
      if ((dataRows as any[]).length === 0) continue;

      const countRes = (await userPool.query(
        `SELECT COUNT(*) AS cnt FROM \`${tName}\``,
      )) as [any[], any];
    ...
    if (!profiles.length)
      return { success: false, error: "All tables are empty." };
    ...
    if (barT) {
      ...
      if (cc.length > 0 && nc.length > 0) {
        ...
        const insight = await getAIInsight(
          `Analyze this dataset from table "${barT.tableName}" showing "${nc[0].name}" grouped by "${cc[0].name}". Data: ${formatDataForPrompt(data.slice(0, 5))}. Write exactly 1-2 sentences highlighting the most important findings from this dataset.`,
        );
        charts.push({
          id: "bar",
          title: `${nc[0].name} by ${cc[0].name}`,
          type: "bar",
          data,
          insight,
          xKey: "name",
        ...
        const data = barT.numericCols.slice(0, 6).map((nc) => {
          const sum = barT.rows.reduce(
            (s: number, r: any) => s + (parseFloat(r[nc.name]) || 0),
            0,
          );
          return { name: nc.name, value: Math.round(sum * 100) / 100 };
        });
        const insight = await getAIInsight(
          `Analyze sums of numeric columns in "${barT.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences about highest and lowest.`,
        );
        charts.push({
          id: "bar",
          title: `Column Sums — ${barT.tableName}`,
          type: "bar",
          data,
          insight,
          xKey: "name",
    ...
    if (pieT) {
      ...
      const insight = await getAIInsight(
        `Analyze this dataset from table "${pieT.tableName}" showing distribution of "${catCol.name}". Data: ${formatDataForPrompt(data)}. Write 1-2 sentences about which category is most and least common with exact counts.`,
      );
      charts.push({
        id: "pie",
        title: `${catCol.name} Distribution — ${pieT.tableName}`,
        type: "pie",
        data,
        insight,
        xKey: "name",
    ...
    if (lineT && lineT.numericCols.length > 0) {
      ...
      const orderCol =
        lineT.temporalCols[0] || lineT.categoricalCols[0] || lineT.columns[0];
      const numCol = lineT.numericCols[0];
      const data = buildLineChart(lineT.rows, orderCol, numCol);
      const insight = await getAIInsight(
        `Analyze this sequential dataset from table "${lineT.tableName}" showing "${numCol.name}" over "${orderCol.name}". Data: ${formatDataForPrompt(data.slice(0, 5))}. Write 1-2 sentences about trends, peaks and dips.`,
      );
      charts.push({
        id: "line",
        title: `${numCol.name} Trend — ${lineT.tableName}`,
        type: "line",
        data,
        insight,
        xKey: "name",
        yKeys: ["value"],
        sourceTable: lineT.tableName,
      });
    }
    ...
    if (scatT && scatT.numericCols.length >= 2) {
      ...
      const data = buildScatterChart(
        scatT.rows,
        scatT.numericCols[0],
        scatT.numericCols[1],
      );
      const insight = await getAIInsight(
        `Analyze this paired dataset from table "${scatT.tableName}" comparing "${scatT.numericCols[0].name}" vs "${scatT.numericCols[1].name}". Sample: ${formatDataForPrompt(data.slice(0, 5))}. Write 1-2 sentences about the highest and lowest data points.`,
      );
      charts.push({
        id: "scatter",
        title: `${scatT.numericCols[0].name} vs ${scatT.numericCols[1].name} — ${scatT.tableName}`,
        type: "scatter",
        data,
        insight,
        xKey: "x",
        yKeys: ["y"],
        sourceTable: scatT.tableName,
      });
    }
    ...
    if (stackT) {
      ...
      if (cc.length >= 2 && nc.length > 0) {
        const { data, subKeys } = buildStackedBarChart(
          stackT.rows,
          cc[0],
          cc[1],
          nc[0],
        ...
        const insight = await getAIInsight(
          `Analyze this hierarchical dataset from "${stackT.tableName}" showing "${nc[0].name}" by "${cc[0].name}" & "${cc[1].name}". Categories: ${subKeys.join(", ")}. Data: ${formatDataForPrompt(data.slice(0, 3))}. Write 1-2 sentences about the largest categories.`,
        );
        charts.push({
          id: "stackedBar",
          title: `${nc[0].name} Breakdown — ${stackT.tableName}`,
          type: "stackedBar",
          data,
          insight,
          xKey: "name",
          yKeys: subKeys,
          sourceTable: stackT.tableName,
        });
        ...
        for (const row of stackT.rows) {
          ...
          for (const n of nc.slice(0, 3))
            grouped[key][n.name] =
              (grouped[key][n.name] || 0) + (parseFloat(row[n.name]) || 0);
        }
        ...
        const data = Object.entries(grouped)
          ...
          .map(([name, vals]) => {
            ...
            for (const sk of subKeys)
              pt[sk] = Math.round((vals[sk] || 0) * 100) / 100;
            return pt;
          });
        const insight = await getAIInsight(
          `Analyze this categorized dataset from "${stackT.tableName}" with metrics ${subKeys.join(", ")} by "${catCol.name}". Data: ${formatDataForPrompt(data)}. Write 1-2 sentences.`,
        );
        charts.push({
          id: "stackedBar",
          title: `Metrics — ${stackT.tableName}`,
          type: "stackedBar",
          data,
          insight,
          xKey: "name",
    ...
    if (
      radarT &&
      radarT.categoricalCols.length > 0 &&
      radarT.numericCols.length >= 2
    ) {
      usedTables.add(radarT.tableName);
      const data = buildRadarChart(
        radarT.rows,
        radarT.categoricalCols[0],
        radarT.numericCols,
      );
      const categoryKeys = Object.keys(data[0] || {}).filter(
        (k) => k !== "name" && k !== "value",
      );
      const insight = await getAIInsight(
        `Analyze this comparative dataset from "${radarT.tableName}" showing averages. Metrics: ${data.map((d) => d.name).join(", ")}. Categories: ${categoryKeys.join(", ")}. Data: ${formatDataForPrompt(data)}. Write 1-2 sentences about the best performing category.`,
      );
      charts.push({
        id: "radar",
        title: `Feature Comparison — ${radarT.tableName}`,
        type: "radar",
        data,
        insight,
        xKey: "name",
        yKeys: categoryKeys,
        sourceTable: radarT.tableName,
      });
      ...
      const insight = await getAIInsight(
        `Analyze averages from "${radarT.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences.`,
      );
      charts.push({
        id: "radar",
        title: `Averages — ${radarT.tableName}`,
        type: "radar",
        data,
        insight,
        xKey: "name",
    ...
    while (charts.length < 6 && fillIdx < profiles.length) {
      ...
      const data = p.columns.slice(0, 6).map((c) => ({
        name: c.name,
        value: p.rows.filter((r: any) => r[c.name] != null).length,
      }));
      const insight = await getAIInsight(
        `Analyze data quality for "${p.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences.`,
      );
      charts.push({
        id: `extra_${charts.length}`,
        title: `Data Quality — ${p.tableName}`,
