  ...
  try {
    ...
    if (barT) {
      ...
      if (cc.length > 0 && nc.length > 0) {
        ...
        const insight = await getAIInsight(`Analyze sums of numeric columns in "${barT.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences about highest and lowest.`);
    ...
    if (stackT) {
      ...
      if (cc.length >= 2 && nc.length > 0) {
        ...
        const insight = await getAIInsight(`Analyze this categorized dataset from "${stackT.tableName}" with metrics ${subKeys.join(", ")} by "${catCol.name}". Data: ${formatDataForPrompt(data)}. Write 1-2 sentences.`);
    ...
    if (radarT && radarT.categoricalCols.length > 0 && radarT.numericCols.length >= 2) {
      ...
      const insight = await getAIInsight(`Analyze averages from "${radarT.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences.`);
    ...
    while (charts.length < 6 && fillIdx < profiles.length) {
      ...
      const insight = await getAIInsight(`Analyze data quality for "${p.tableName}": ${formatDataForPrompt(data)}. Write 1-2 sentences.`);
      charts.push({
